...
trait Utility
{
    ...
    {
        $enames = REDCap::getEventNames();
        $enames_flip = REDCap::getEventNames(true);
        $eventNames = array_values((is_bool($enames) ? [] : $enames) ?? []);
        foreach (array_values((is_bool($enames_flip) ? [] : $enames_flip) ?? []) as $i => $unique)
